...
const baseOptions: Options = {
  token: 'token_123',
  repository: 'step-security/pytest-coverage-comment',
  commit: 'abc123',
  prefix: '',
  pathPrefix: '',
  covFile: '',
  ...
  maxFailedTests: 30,
  multipleFiles: [],
  repoUrl: 'https://github.com/step-security/pytest-coverage-comment',
};
...
describe('failedTestsToMarkdown', () => {
  ...
  test('should link classname to the test file from traceback location', () => {
    ...
    expect(relative).toContain(
      '<a href="https://github.com/step-security/pytest-coverage-comment/blob/abc123/backend/tests/test_service.py#L25">tests.test_service</a> › test_one',
    );
  ...
  test('should render real pytest fixture end to end', () => {
    ...
    expect(html).toContain(
      '<a href="https://github.com/step-security/pytest-coverage-comment/blob/abc123/tests/test_service.py#L12">tests.test_service</a> › test_wrong_title',
    );
    expect(html).toContain(
      "<code>AssertionError: assert 'first post' == 'my first post'</code>",
    );
